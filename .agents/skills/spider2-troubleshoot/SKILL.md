---
name: spider2-troubleshoot
description: 'Debug Spider2 failures and analyze data quality. Triggers: crawl ERROR, pueue task failed, item_scraped_count=0, empty fields (condition_list/title/series_list), data quality check, parquet analysis, increment data issues.'
metadata:
  version: '1'
---
...
```bash
# 查看实际运行配置
rg "<spider_name>" .mise/tasks/spider2/run-crawl-all.sh
...
| 误判 | 实际情况 | 如何避免 |
| ------ | ---------- | ---------- |
| "dorasuta 需要加 cloudbypass 代理" | dorasuta 是 IP 白名单，直连即可 | 检查 run-crawl-all.sh |
