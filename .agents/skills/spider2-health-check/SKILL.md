---
name: spider2-health-check
description: 'Daily proactive check for Spider2 data integrity. Triggers: routine health check, detect missing crawlers, row count drops, scheduled validation.'
metadata:
  version: '1'
---
...
```bash
mise run spider2:health-check 2>&1 | tee /tmp/health-check.log
mise run spider2:health-check --date 260114 2>&1 | tee /tmp/health-check.log
mise run spider2:health-check --baseline 260110 2>&1 | tee /tmp/health-check.log
mise run spider2:health-check --history 30 2>&1 | tee /tmp/health-check.log  # 检查30天历史
```
...
**运行时配置看 `run-crawl-all.sh`，不是爬虫代码。**

爬虫代码中的 `DOWNLOADER_MIDDLEWARES` 只是声明支持代理，实际是否启用取决于命令行参数 `-s JHS_PROXY=cloudbypass`。

```bash
# 查看运行时配置
rg "dorasuta|hareruya|cardrush" .mise/tasks/spider2/run-crawl-all.sh
```
...
**判断方法**：检查 `run-crawl-all.sh` 中是否有 `-s JHS_PROXY=cloudbypass`
...
| 误判 | 实际情况 | 如何避免 |
| ------ | ---------- | ---------- |
| "CloudBypass 余额不足" | dorasuta 不走 CloudBypass API | 检查 run-crawl-all.sh 确认代理配置 |
...
```bash
mise run spider2:test-url
mise run spider2:test-url --url "https://dorasuta.jp/pokemon-card/product-list?cocd=1"
mise run spider2:test-url --url "https://www.hareruya2.com/collections/47" --proxy
```
...
| 症状 | 原因 | 修复 |
| :----- | :----- | :----- |
| "No collections matched filters" | `--include` regex 匹配 collection 名，但 collection 名固定 | 检查 `run_increment_dorasuta.py` 的 `--include` 参数 |
...
```bash
# 重新导出指定批次
mise exec --env local -- uv run apps/spider2/scripts/run_increment_dorasuta.py export --run-id 260115_080001
```
