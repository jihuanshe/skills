---
name: tmux
description: 'Remote control tmux sessions for interactive CLIs. Triggers: tmux, interactive shell, REPL.'
metadata:
  version: '2'
---
...
tmux -S "$SOCKET" send-keys -t "$SESSION:$PANE" -- 'python3 -q' Enter
tmux -S "$SOCKET" capture-pane -p -J -t "$SESSION:$PANE" -S -200  # watch output
...
- **NEVER hardcode `:0.0`** — user config may set `base-index 1` / `pane-base-index 1`. After creating a session, ALWAYS query the actual pane target:

  ```bash
  PANE=$(tmux -S "$SOCKET" list-panes -t "$SESSION" -F '#{window_index}.#{pane_index}' | head -1)
  # Then use "$SESSION:$PANE" as the target
  ```
...
- `-S`/`--socket` tmux socket path (**required when using private sockets** — omitting this is the #1 cause of "timed out but text was there")
