...
- `amp skill add jihuanshe/skills/o11y-skills/logfire --target .agents/skills --overwrite`
...
- 读取模板 `mcp.tpl.json`（安装后位于 `.agents/skills/logfire/mcp.tpl.json`）
...
- 获得真实的 Token 后替换模板后保存同路径的 `mcp.json`（如 `.agents/skills/logfire/mcp.json`）
